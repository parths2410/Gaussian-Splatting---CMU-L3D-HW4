...
def colours_from_spherical_harmonics(spherical_harmonics, gaussian_dirs):
    ...
    # Initialize colours with the zeroth-order harmonic
    colours = SH_C0 * spherical_harmonics[:, :3]  # f_dc_*

    # First order harmonics
    if spherical_harmonics.size(1) > 3:
        c1 = spherical_harmonics[:, 3:6]
        c2 = spherical_harmonics[:, 6:9]
        c3 = spherical_harmonics[:, 9:12]
        colours -= SH_C1 * y.unsqueeze(1) * c1
        colours += SH_C1 * z.unsqueeze(1) * c2
        colours -= SH_C1 * x.unsqueeze(1) * c3

    # Second order harmonics
    if spherical_harmonics.size(1) > 12:
        ...
        xx = x * x
        yy = y * y
        colours += SH_C2_0 * xy.unsqueeze(1) * c4
        colours += SH_C2_1 * yz.unsqueeze(1) * c5
        colours += SH_C2_2 * (2.0 * zz - xx - yy).unsqueeze(1) * c6
        colours += SH_C2_3 * xz.unsqueeze(1) * c7
        colours += SH_C2_4 * (xx - yy).unsqueeze(1) * c8

    # Third order harmonics
    if spherical_harmonics.size(1) > 27:
        ...
        colours += SH_C3_0 * y * (3.0 * xx - yy).unsqueeze(1) * c9
        colours += SH_C3_1 * xy * z.unsqueeze(1) * c10
        colours += SH_C3_2 * y * (4.0 * zz - xx - yy).unsqueeze(1) * c11
        colours += SH_C3_3 * z * (2.0 * zz - 3.0 * xx - 3.0 * yy) * c12
        colours += SH_C3_4 * x * (4.0 * zz - xx - yy) * c13
        colours += SH_C3_5 * z * (xx - yy) * c14
        colours += SH_C3_6 * x * (xx - 3.0 * yy) * c15
