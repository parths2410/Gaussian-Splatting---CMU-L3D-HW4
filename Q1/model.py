...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            cov_3D = torch.matmul(M, M.transpose(1, 2))  # (N, 3, 3)
    ...
    def compute_cov_2D(
        ...
        # of this function?
        W = camera.get_world_to_view_transform().get_matrix() # (1, 4, 4), transformation_matrix
        W = W[:, :3, :3] # (1, 3, 3), rotation_matrix
        ...
        # paper equation (5)
        T = torch.matmul(J, W)  # (N, 2, 3)
        cov_2D = torch.matmul(T, torch.matmul(cov_3D, T.transpose(1, 2)))  # (N, 2, 2)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        # Matrix multiplication with cov_2D_inverse
        # (N, H*W, 2) x (N, 2, 2) -> (N, H*W, 2)
        # diff_cov = torch.matmul(diff, cov_2D_inverse)

        # Compute squared Euclidean distance
        # (N, H*W)
        # squared_distance = torch.sum(diff_cov * diff, dim=-1)

        # Multiply by -0.5 to match the exponent form
        power = -0.5 * diff @ cov_2D_inverse * diff  # (N, H*W, 2) x (N, 2, 2) x (N, 2, H*W)
        power = power.sum(-1)
...
class Scene:
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        # print(opacities.shape, exp_power.shape)
