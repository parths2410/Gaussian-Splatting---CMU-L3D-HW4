...
def run_training(args):
    ...
    for itr in range(args.num_itrs):
        ...
        pred_img, _, _ = scene.render(camera, args.gaussians_per_splat, img_size, bg_colour)

        # Compute loss
        ### YOUR CODE HERE ###
        # HINT: A simple standard loss function should work.
        l1_loss = torch.nn.functional.l1_loss(pred_img, gt_img)
        # dssim_loss = 1.0 - torch.nn.functional.ssim(pred_img, gt_img, data_range=1.0)
        dssim_loss = 0.0

        lmda = 0.0
        loss = l1_loss + lmda * dssim_loss
